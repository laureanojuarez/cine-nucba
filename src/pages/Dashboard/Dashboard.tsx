...
export default function Dashboard() {
  ...
  if (!token || !user) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-gray-700 py-10 px-2">
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-gray-700 py-10 px-2">
      <div className="bg-white/90 rounded-2xl shadow-2xl p-8 w-full max-w-2xl flex flex-col items-center border border-gray-200">
        {user.role === "admin" && (
          <Link
            to={"/admin"}
            className="mb-4 bg-gradient-to-r from-green-500 to-green-700 text-white px-4 py-2 rounded-lg hover:scale-105 transition font-semibold shadow"
          >
            Panel de control
          </Link>
        )}
        ...
        {entradas.length === 0 ? (
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
            {entradas.map((reserva) => (
              <li
                key={reserva.id}
                className="bg-gradient-to-br from-green-100 via-white to-blue-100 border border-green-200 rounded-xl p-4 flex flex-col gap-2 shadow hover:scale-[1.02] transition"
              >
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-2xl">🎟️</span>
                  <span className="font-bold text-green-700 text-lg">
                    {reserva.funcion?.movie?.titulo || "Película"}
                  </span>
                </div>
                <div className="text-gray-700">
                  <span className="font-semibold">Sala:</span>{" "}
                  {reserva.funcion?.sala?.nombre || "N/A"}
                </div>
                <div className="text-gray-700">
                ...
                <div className="text-gray-700">
                  ...
                  {reserva.funcion?.fecha} - {reserva.funcion?.horario}
                </div>
                <div className="text-gray-700">
